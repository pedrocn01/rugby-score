...
- **Global scope**: `C:/src/Proyectos/rugby_app/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/src/Proyectos/rugby_app/.claude/notes/`)
...
2. Glob `C:/src/Proyectos/rugby_app/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/src/Proyectos/rugby_app/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/src/Proyectos/rugby_app/.claude/notes/` — works in any directory
