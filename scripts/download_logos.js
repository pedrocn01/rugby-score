...
function downloadOnce(url, dest) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(url, {
      headers: {
        // Wikimedia requiere User-Agent con contacto para bots automatizados
        'User-Agent': 'RugbyScoreApp/1.0 (pedrocastronevares@gmail.com) node-https',
        'Accept': 'image/png,image/jpeg,image/*',
      }
    }, res => {
      if (res.statusCode === 301 || res.statusCode === 302) {
        file.close(); try { fs.unlinkSync(tmp); } catch {}
        downloadOnce(res.headers.location, dest).then(resolve).catch(reject);
        return;
      }
...
async function download(url, dest) {
  ...
  for (let attempt = 0; attempt <= delays.length; attempt++) {
    try {
      ...
      if (e.message === '429' && attempt < delays.length) {
...
async function main() {
  ...
  console.log(`Descargando ${entries.length} logos en ${OUTPUT_DIR}...\n`);
  ...
  for (const [name, url] of entries) {
    ...
    try {
      ...
      failed.push({ name, url, err: e.message });
    }
    await new Promise(r => setTimeout(r, 5000)); // pausa de 5s para respetar Wikimedia
  }

  console.log(`\n${'='.repeat(50)}`);
  console.log(`OK: ${ok.length}   Fallidos: ${failed.length}`);
  if (failed.length) {
    console.log('\nLogos que fallaron:');
    failed.forEach(({ name, url, err }) => console.log(`  ${name}: ${err}\n    ${url}`));
  }
  console.log('\nListo.');
}
